...
“mean” beyond COCO’s per-category average). Top-level **`recall`** is only COCO AR
(maxDets=100, IoU 0.50:0.95), same as ``stats[8]``; greedy recall only in **`recall_iou25`**,
**`recall_iou50`**, …

**COCO-official** AR at IoU 0.25 / 0.50 / 0.75 → ``coco_ar_iou*``; **COCO** precision /
FDR on the official PR tensor at ``--coco-pr-recall`` → ``coco_precision_r*_iou*`` /
``coco_fdr_r*_iou*``. Greedy micro P/R/FDR at ``--precision-score-thr`` (det kept iff
``score >= thr``) and per-IoU from ``--greedy-iou-thrs`` → ``precision_iou*`` /
``recall_iou*`` / ``fdr_iou*`` with **integer percent** suffixes (25, 50, 75). Legacy
``precision`` / ``fdr`` = greedy @ ``--precision-iou-thr``. Greedy matching skips
``iscrowd==1`` GT and requires matching ``category_id``. ``summarize()`` is never called
with unsupported kwargs (quiet mode uses ``redirect_stdout``). No FPS.
...
def main() -> None:
    ...
    print(
        ...
        "See docs/benchmark_metrics_schema.md in study repo.)",
        file=sys.stderr,
    )
